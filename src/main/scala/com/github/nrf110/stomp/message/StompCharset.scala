...
// adapted from
// https://github.com/spray/spray/blob/6919e8faf6d5f148d703641b21214e5e025b128d/spray-http/src/main/scala/spray/http/HttpCharset.scala
...
sealed abstract class StompCharsetRange {
  def value: String
  def matches(charset: StompCharset): Boolean
  override def toString = "StompCharsetRange(" + value + ')'
}

sealed abstract class StompCharset extends StompCharsetRange {
  val nioCharset: Charset = Charset.forName(value)
  def aliases: Seq[String]
  def matches(charset: StompCharset) = this == charset
  override def equals(obj: Any) = obj match {
    case x: StompCharset ⇒ (this eq x) || value == x.value
    case _               ⇒ false
  }
  override def hashCode() = value.##
  override def toString = "StompCharset(" + value + ')'
}

// see http://www.iana.org/assignments/character-sets
object StompCharsets extends ObjectRegistry[String, StompCharset] {

  def register(charset: StompCharset): StompCharset = {
    register(charset.value.toLowerCase, charset)
    charset.aliases.foreach(alias ⇒ register(alias.toLowerCase, charset))
    charset
  }

  object `*` extends StompCharsetRange {
    def value = "*"
    def matches(charset: StompCharset) = true
  }

  private class PredefCharset(val value: String, val aliases: String*) extends StompCharset
  ...
  class CustomStompCharset private (val value: String, val aliases: Seq[String]) extends StompCharset
  object CustomStompCharset {
    def apply(value: String, aliases: Seq[String] = Nil): Option[CustomStompCharset] = {
      try {
        Some(new CustomStompCharset(value.toLowerCase, aliases))
      } catch {
