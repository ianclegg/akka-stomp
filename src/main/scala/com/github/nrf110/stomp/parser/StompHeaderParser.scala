...
// adapted from
// https://github.com/spray/spray/blob/6919e8faf6d5f148d703641b21214e5e025b128d/spray-can/src/main/scala/spray/can/parsing/HttpHeaderParser.scala
...
import com.github.nrf110.stomp.message.StompHeaders.RawHeader
import com.github.nrf110.stomp.{SingletonException, ErrorInfo}
import CharUtils._
...
/**
 * Provides for time- and space-efficient parsing of a STOMP header line in an STOMP message.
 * It keeps a cache of all headers encountered in a previous request, so as to avoid reparsing and recreation of header
 * model instances.
 * For the life-time of one STOMP connection an instance of this class is owned by the connection, i.e. not shared
 * with other connections. After the connection is closed it may be used by subsequent connections.
 */
final class StompHeaderParser private (
   ...
   */
  var resultHeader: StompHeader = EmptyHeader
  ...
  def copyWith(warnOnIllegalHeader: ErrorInfo ⇒ Unit) =
    new StompHeaderParser(settings, warnOnIllegalHeader, nodes, nodeCount, branchData, branchDataCount, values, valueCount)
  ...
  @tailrec private def parseHeaderValue(input: CompactByteString, valueStart: Int, branch: ValueBranch)(cursor: Int = valueStart, nodeIx: Int = branch.branchRootNodeIx): Int = {
    ...
    if (char == node) // fast match, descend
      ...
      case msb ⇒ node & 0xFF match {
        case 0 ⇒ // leaf node
          resultHeader = values(msb - 1).asInstanceOf[StompHeader]
          cursor
...
object StompHeaderParser {
  import SpecializedHeaderValueParsers._

  object EmptyHeader extends StompHeader {
    def name = ""
    override def lowercaseName = ""
    def value = ""
    override def toString = "EmptyHeader"
  }

  //TODO: Can anymore headers be pre-defined (i.e. accept-version or version)?
  ...
  def apply(settings: ParserSettings, warnOnIllegalHeader: ErrorInfo => Unit = defaultIllegalHeaderWarning,
            unprimed: Boolean = false): StompHeaderParser = {
    val parser = new StompHeaderParser(settings, warnOnIllegalHeader)
    if (!unprimed) {
      val valueParsers: Seq[HeaderValueParser] =
        StompParser.headerNames.map { name ⇒
          modelledHeaderValueParser(name, StompParser.parserRules(name.toLowerCase), parser.settings.maxHeaderValueLength,
  ...
  def modelledHeaderValueParser(headerName: String, parserRule: Rule1[StompHeader], maxHeaderValueLength: Int,
                                maxValueCount: Int) =
    new HeaderValueParser(headerName, maxValueCount) {
      def apply(input: CompactByteString, valueStart: Int, warnOnIllegalHeader: ErrorInfo ⇒ Unit): (StompHeader, Int) = {
        ...
        val header = StompParser.parse(parserRule, trimmedHeaderValue) match {
          case Right(h) ⇒ h
          case Left(error) ⇒
  ...
  @tailrec private def scanHeaderNameAndReturnIndexOfColon(input: CompactByteString, start: Int,
    ...
    else fail(s"STOMP header name exceeds the configured limit of ${maxHeaderNameEndIx - start} characters")
  ...
  @tailrec private def scanHeaderValue(input: CompactByteString, start: Int, maxHeaderValueEndIx: Int)(sb: JStringBuilder = null, ix: Int = start): (String, Int) = {
    ...
    else fail(s"STOMP header value exceeds the configured limit of ${maxHeaderValueEndIx - start} characters")
  }

  def fail(summary: String) = throw new ParsingException(summary)
